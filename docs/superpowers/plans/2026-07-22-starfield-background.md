...
// 现有 IIFE 语义：overlayOpacity = 1 - sliderVal/100，值越大遮罩越透（照片越清晰）。
// 对 canvas 再反一次（1 - var）还原成"值越大星空越清晰"：
