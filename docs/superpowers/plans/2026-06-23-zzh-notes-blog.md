...
Run: `node --version`
Expected: 输出 `v20.x.x` 或更高（本地实测 v24.14.0 亦可）。GitHub Actions runners 自 2025-09 起已弃用 Node 20、强制跑在 Node 24 上（仅产生 deprecation 警告，不影响构建）。本地与 CI 版本差异不构成阻塞——Hexo 构建产物只依赖配置与依赖锁版本（`package-lock.json`），不依赖运行时版本。
...
> **⚠️ 依赖说明（实测必需，缺一不可）：**
> - `hexo-generator-index/archive/category/tag`：Hexo 7 核心**只内置** asset/page/post 三个 generator，首页/归档/分类/标签页必须靠这四个独立插件生成，否则对应 HTML 不会产出。
> - `hexo-renderer-marked`：Markdown→HTML 渲染器，不装则文章正文无法渲染。
> - `hexo-renderer-stylus`：NexT 的 `.styl` 样式编译器。
> - `css` + `@adobe/css-tools`：NexT 8.27 的脚本直接 `require('css')`/`require('@adobe/css-tools')`，但它的 `package.json` 没声明 `dependencies`（只有 devDependencies），必须由宿主项目显式安装。
> - `@next-theme/plugins`：NexT vendor 资源内联处理（缺失不致命，但装上更稳）。
> - **绝对不要装 `hexo-renderer-nunjucks`**：该包的实现是残缺的（`nunjucks.configure({})` 不传路径），会覆盖 Hexo 7 内置的完整 Nunjucks 渲染器（内置版会 `configure(dirname(path))`），导致 `{% extends '_layout.njk' %}` 报 "template not found"、首页/归档页无法生成。Hexo 7 已自带完整版，无需外部包。
...
- [ ] **Step 3: 启用 GitHub Pages（workflow 模式）——必须在推送前完成**

> **⚠️ 实测：新仓库默认未启用 Pages。若直接推送，workflow 会因 `pages: write` 权限无处落地而 `startup_failure`，连 job 日志都没有。必须先用 API 把 Pages source 设为 `workflow`。**

Run:
```bash
gh api -X POST repos/zzh-learner/zzh-notes/pages -f build_type=workflow -f source[branch]=main
```
Expected: 返回 JSON，含 `"build_type":"workflow"` 与 `"html_url":"https://zzh-learner.github.io/zzh-notes/"`。

- [ ] **Step 4: 推送 main 分支（触发首次 CI）**

Run: `git push -u origin main`
Expected: 推送成功，自动触发 deploy workflow。

- [ ] **Step 5: 监控首次 CI 运行**

推送后等约 10 秒让 run 注册，然后：
```bash
gh run list --limit 1 --json databaseId   # 取最新 run-id
gh run watch <run-id> --exit-status --interval 15
```
Expected: `build` job（约 40s）→ `deploy` job（约 8s）依次变绿，`deploy` 输出 Pages URL。

如失败：`gh run view <run-id> --log-failed` 查看日志。常见原因仍是 Pages 未启用（回 Step 3）。

> 注：CI 会有一条 "Node.js 20 is deprecated... forced to run on Node.js 24" 的 annotation 警告（GitHub 2025-09 变更），无害，构建照常成功。
